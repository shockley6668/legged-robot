...
void INS_task(void) {
  ...
  while (1) {
    ...
    {
      INS.MotionAccel_b[i] =
          (INS.Accel[i] - gravity_b[i]) * ins_dt / (INS.AccelLPF + ins_dt) +
          INS.MotionAccel_b[i] * INS.AccelLPF / (INS.AccelLPF + ins_dt);
      //			INS.MotionAccel_b[i] = (INS.Accel[i] ) * dt /
      //(INS.AccelLPF + dt)
      //														+
    ...
    if (ins_time > 3000.0f) {
      ...
      // Send IMU Data (MSGID 0x04) at 100Hz
      static uint8_t imu_send_cnt = 0;
      if (++imu_send_cnt >= 10) {
        uint8_t imu_payload[40];
        memcpy(&imu_payload[0], (uint8_t *)INS.q, 16);
        memcpy(&imu_payload[16], (uint8_t *)INS.Gyro, 12);
        memcpy(&imu_payload[28], (uint8_t *)INS.Accel, 12);
        mavlink_send_msg(0x04, imu_payload, 40);
        imu_send_cnt = 0;
      }
    } else {
      ins_time++;
