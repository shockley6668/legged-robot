...
void pstwo_task(void) {
  ...
  while (1) {
    ...
    PS2_data_process(&ps2data, &chassis_move,
                     (float)PS2_TIME / 1000.0f); // 处理数据，设置期望数据
...
void PS2_Cmd(uint8_t CMD) {
  ...
  for (ref = 0x01; ref < 0x0100; ref <<= 1) {
    if (ref & CMD) {
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_2, GPIO_PIN_SET); // 输出一位控制位
                                                          // DO_H;
    } else
...
void PS2_data_process(ps2data_t *data, chassis_t *chassis, float dt) {
  ...
  } else if (data->last_key != 4 && data->key == 4 &&
             chassis->start_flag == 1) {
    // 手柄上的Start按键被按下
  ...
  if (data->last_key != 1 && data->key == 1 && chassis->prejump_flag == 0 &&
      chassis->start_flag == 1) {
    // 手柄上的select按键按一次置1
    chassis->prejump_flag = 1; // 预跳跃标志置1
  } else if (data->last_key != 1 && data->key == 1 &&
             chassis->prejump_flag == 1) {
    // 手柄上的select按键再按一次置0
    chassis->prejump_flag = 0;
  }

  if (data->last_key != 5 && data->key == 5 && chassis->prejump_flag == 1 &&
      chassis->jump_flag == 0 && chassis->jump_flag2 == 0) {
  ...
  if (chassis->start_flag == 1) {                           // 启动
    chassis->v_set = ((float)(data->ry - 128)) * (-0.008f); // 往前大于0
    chassis->x_set = chassis->x_set + chassis->v_set * dt;

    chassis->turn_set =
        chassis->turn_set + (data->rx - 127) * (-0.0005f); // 往右大于0

    chassis->roll_set =
        chassis->roll_set + ((float)(data->lx - 127)) * (-0.00007f);

    mySaturate(&chassis->roll_set, -0.40f, 0.40f);

    chassis->leg_set =
        chassis->leg_set + ((float)(data->ly - 128)) * (-0.000016f);
    mySaturate(&chassis->leg_set, 0.072f, 0.21f);

    if (fabsf(chassis->last_leg_set - chassis->leg_set) >
        0.0001f) { // 遥控器控制腿长在变化
      right.leg_flag =
          1; // 为1标志着遥控器在控制腿长伸缩，根据这个标志可以不进行离地检测，因为当腿长在主动伸缩时，离地检测会误判为离地了
      left.leg_flag = 1;
    }
    chassis->last_leg_set = chassis->leg_set;
  } else if (chassis->start_flag == 0) {    // 关闭
