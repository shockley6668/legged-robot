...
/* USER CODE BEGIN PV */
/* Private variables ---------------------------------------------------------*/
extern void mavlink_handle_msg(uint8_t *data, uint32_t len);
/* USER CODE END PV */
...
static int8_t CDC_Receive_HS(uint8_t *Buf, uint32_t *Len) {
  /* USER CODE BEGIN 11 */
  // Direct processing in ISR to save 33KB RAM (no RX queue)
  // Parse MAVLink frame and control motor immediately
  extern chassis_t chassis_move;

  if (*Len >= 12 && Buf[0] == 0xFD) { // MAVLink v2 STX
    mavlink_handle_msg(Buf, *Len);
  }
